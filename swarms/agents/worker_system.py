...
class TaskQueue:
    """Simple optimistic-concurrency task queue backed by sqlite."""
    ...
    def reclaim_stuck_tasks(self, max_age_seconds: float) -> int:
        """Release tasks that were claimed longer than max_age_seconds.
        ...
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
            # only reclaim tasks that are still pending but claimed
            c.execute(
                "SELECT id FROM tasks WHERE status='pending' AND claimed_at IS NOT NULL AND claimed_at<?",
                (cutoff,),
            )
            rows = c.fetchall()
            ...
            if not ids:
                return 0
            c.execute(
                "UPDATE tasks SET claimed_by=NULL, claimed_at=NULL WHERE id IN ({seq})".format(
                    seq=",".join(["?"] * len(ids))
                ),
                ids,
            )
    ...
    def reset_all_claims(self) -> int:
        """Reset claimed_by/claimed_at for all pending tasks. Returns number of rows updated."""
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
            c.execute("SELECT id FROM tasks WHERE status='pending' AND claimed_by IS NOT NULL")
            rows = c.fetchall()
            ids = [r[0] for r in rows]
            if not ids:
                return 0
    ...
    def claim_task(self, worker_id: str) -> Optional[dict]:
        # optimistic claim: set claimed_by where status is pending and claimed_by is NULL
        with sqlite3.connect(self.db_path, timeout=10) as conn:
            c = conn.cursor()
            now = time.time()
            # pick the oldest pending unclaimed task
            c.execute(
...
class Worker:
    """Focused worker that claims one task at a time and pushes changes to git."""
    ...
    def __init__(
        ...
        executor: Callable[[dict, str], dict],
        max_retries: int = 5,
        timeout: int = 60 * 30,
            ...
            try:
                if self.telemetry:
                    try:
                        ...
                        pass
                # execute assigned work (user provided function)
                result = self.executor(payload, self.repo_path)

                # try to push result to branch
                ...
                if ok:
                    ...
                    # conflict handling: refresh, optionally re-plan locally
                    if info.get("conflict"):
                        ...
                        # try to rebase and continue
                        self.git.fetch()
                        self.git.rebase(branch)
                        continue
                    else:
                        # unrecoverable push failure
...
class JudgeAgent:
    """Cycle manager that inspects tasks and decides whether to continue or trigger fresh start."""
    ...
    def decide(self) -> bool:
        ...
        # simple policy: if failure ratio above threshold, trigger fresh start
    ...
    def fresh_start(self, git_client, reset_branch: str = "master") -> dict:
        """Perform a fresh start on the repo and reset pending task claims.
        ...
        try:
            # reset any pending claims so workers can start fresh
