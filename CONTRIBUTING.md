...
3. Run the command `pre-commit run --all-files`. This will execute the pre-commit hooks configured for this project against the modified files. If any issues are found, the pre-commit tool will provide feedback on how to resolve them. Review the feedback and make the necessary changes to your files. After making the changes, re-run the pre-commit command until all issues are resolved.
...
All new functions and classes in `swarms` are required to include docstrings. This is a prerequisite for any new functions and classes to be added to the library.

`swarms` adheres to the [Google Python docstring style](https://google.github.io/styleguide/pyguide.html). Please refer to the style guide while writing docstrings for your contribution.
...
All PRs must pass all tests and linting requirements before they are merged.
